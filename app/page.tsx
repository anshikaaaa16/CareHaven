...
export default function Home() {
  ...
  const isSignedIn = !!session;           // <-- And this

  const welcomeSteps = [
    {
      title: "Hi, I'm CareHaven 👋",
      description:
    ...
    <div className="flex flex-col min-h-screen overflow-hidden">
      ...
      <section className="relative min-h-[90vh] mt-20 flex flex-col items-center justify-center py-12 px-4">
        ...
        >
          ...
          Find comfort in every conversation. Our website offers a safe space to talk, reflect, and heal—whenever you need it most.
          </p>
...
<div className="text-center mt-4 transition-all duration-500 ease-in-out">
  <p className="text-md font-medium text-foreground">
    {
      emotion < 20
        ...
        : emotion < 45
        ? "Glad you’re feeling steady. Want to explore mindfulness?"
        : emotion < 70
        ? "Peace is powerful. Let’s build on that."
        : emotion < 90
        ? "You're glowing! Let's keep that energy up 🌟"
        : "Feeling excited? Great things await!"
    }
  </p>
  {/* Show CTA if user isn't signed in */}
  {!isSignedIn && (
    <p className="text-sm text-muted-foreground mt-2 animate-pulse">
      <Link href="/sign-in" className="underline hover:text-primary">
        Sign in
      </Link>{" "}
      to explore personalized tools and guidance.
    </p>
  )}
</div>
  ...
  <motion.div className="text-center mb-16 space-y-4 text-white">
    <h2 className="text-3xl font-bold bg-gradient-to-r from-primary/90 to-primary bg-clip-text text-transparent dark:text-primary/90">
      Why People Trust CareHaven
    </h2>
    <p className="text-foreground dark:text-foreground/95 max-w-2xl mx-auto font-medium text-lg">
      CareHaven blends compassionate AI with thoughtful design to help you feel heard, understood, and supported—anytime you need it.
    </p>
  </motion.div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative">
    {features.map((feature, index) => (
      <motion.div
        ...
        viewport={{ once: true }}
      >
        <Card className="group relative overflow-hidden border border-primary/10 hover:border-primary/20 transition-all duration-300 h-[220px] bg-card/40 dark:bg-card/80 backdrop-blur-md shadow-md">
          <div
            className={`absolute inset-0 bg-gradient-to-br ${feature.color} to-transparent opacity-0 group-hover:opacity-25 transition-opacity duration-500 dark:group-hover:opacity-35`}
          />
